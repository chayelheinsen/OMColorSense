...
- (id)init
{
	if (self = [super init]) {
        ...
        _hexColorRegex = [NSRegularExpression regularExpressionWithPattern:@"\\[(UI|NS)Color\\s*colorWithHex:@\"(?:#)?(?:[0-9a-fA-F]{2}){2,4}|(?:#)?(?:[0-9a-fA-F]{1}){3,4}\"\\]" options:0 error:NULL];
	}
	return self;
}
	...
	if ([[notification object] isKindOfClass:NSClassFromString(@"DVTSourceTextView")] && [[notification object] isKindOfClass:[NSTextView class]]) {
		...
		if (selectedRanges.count >= 1) {
			...
			if (matchedColor) {
                ...
                NSRect selectionRectOnScreen = [self.textView firstRectForCharacterRange:self.selectedColorRange actualRange:NULL];
    ...
    if (!foundColor) {
        [_hexColorRegex enumerateMatchesInString:text options:0 range:NSMakeRange(0, text.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
            ...
            if (selectedRange.location >= colorRange.location && NSMaxRange(selectedRange) <= NSMaxRange(colorRange)) {
                ...
                [scanner setScanLocation:colorRange.location];
                [scanner scanUpToString:@"\"" intoString:nil];
                [scanner setScanLocation:scanner.scanLocation + 1];
                [scanner scanUpToString:@"\"" intoString:&color];
                foundColor = [OMColorHelper colorWithHex:color];
                //[UIColor colorWithHex:@"ff00ad"];
